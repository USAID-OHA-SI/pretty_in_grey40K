...
caption <- paste0("Source: Testing data from glitr package | Created on: ", 
                          Sys.Date())
```
...
```{r hts}
  ...
  # In many cases, making KPI's may be easier in Google Presentation/PowerPoint.
    si_save(file.path(images, "kpi"), plot = p, width = 8, height = 0.75)
...
```{r kpi_exmple, echo = FALSE, out.width = '100%'}
  knitr::include_graphics(here("Images/kpi.png"))  
```
...
# Match axis label colors to top to performers
  label_colors <- if_else(sort(hts_bar_sorted$yield, decreasing = FALSE) > 0.2, scooter, color_caption)
...
```{r}
 ...
 slope_plot <-  hts_slope %>% 
  ...
  si_save(here(images, "slope_plot_labeled"), plot = slope_plot, scale = 1.5)
  
```

```{r slope plot labeled, echo = FALSE, out.width = '100%'}
  knitr::include_graphics(here("Images/slope_plot_labeled.png"))  
```
...
hts_tgt_achieved <- 
  ...
  mutate(achieved = if_else(achievement > 1, genoa, grey10k),
          `Targets Achieved` = 1) %>% 
  ggplot(aes(x = `Targets Achieved`, y = fct_reorder(primepartner, achievement), fill = achieved)) +
...
ach_color <- if_else(sort(hts_dev_wide$achievement, decreasing = FALSE) > 1, genoa, color_title)

hts_ach_table <-  
  hts_dev_wide %>% 
  mutate(achieved = if_else(achievement > 1, genoa, color_title),
          `Results / Targets` = 1) %>% 
  ggplot(aes(x = `Results / Targets`, y = fct_reorder(primepartner, achievement), fill = grey70k)) +
    geom_text(aes(label = paste0(comma(cumulative), " / ", comma(targets)), color = achieved)) +
      scale_color_identity() +
    si_style_nolines() +
    scale_x_discrete(position = "top") +
...
hts_ach_table + hts_tgt_achieved + hts_ach + plot_layout(widths = c(2, 1, 6))
```
...
# Create basic scatter plot
  colors_length <- unique(hts$modality) %>% length()
