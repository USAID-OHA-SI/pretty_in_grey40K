...
The following tutorial provides a crash course for using the SIEI recommended chart types and color palettes. The examples correspond to those outlined in the Data Viz Cheat Sheet. All plots are created using the ggplot2, glitr and a few helper packages such as scales, ggrepel and sf. To complete this tutorial, you will need to download and install all of these packages.
...
# Create geom_point plot that shows whether or not a partner has achieved targets
  hts_tgt_achieved <- 
    ...
    ggplot(aes(x = `Targets Achieved`, 
               ...
               fill = achieved)) +
    geom_point(shape = 21, size = 4, color = genoa) +
    scale_fill_identity() +
    si_style_nolines() + 
    ...
    theme(axis.text.y =  element_blank(),
          axis.title.y = element_blank())
  hts_tgt_achieved
  si_save(here(pub_images, "ach_circle_middle_2.png"), plot = hts_tgt_achieved)
```
...
# This will be used to fill in the axis labels with different colors
  ...
  si_save(here(pub_images, "hts_gap_middle_1.png"), plot = hts_gap_table)
  ...
  si_save(here(pub_images, "hts_table_lhs.png"), plot = hts_ach_table)
...
###Spatial
Maps convey geographic patterns of data using coordinates and polygons. Maps are using for showing spatial correlations. Combining a map with a bar graph can be a powerful way to show how a metric is distributed by magnitude as well as across space. In the example below, we walk through how to create a choropleth map and combine it with a bar graph. As before, we use the `hts` data available from the `glitr` package.

```{r}
# Create a cross walk to join partner data to a selected polygon
...
# Spread the data to make the acheivement calculations a bit easier
 ...
 # Unfortunately, LEO did not report on time so they are dropped
  hts_geo_dev <- left_join(hts_geo %>% filter(primepartner != "Leo"), hts_dev_wide)
...
map_rhs <- 
    hts_geo_dev %>% 
    ggplot() +
    geom_sf(aes(fill = achievement), color = "white", stroke = 0.5, alpha = 0.85,
            ) +
    geom_sf(data = hts_geo_admin0, color = grey90k, stroke = 2, fill = NA) +
  scale_fill_si(palette = "denims", discrete = FALSE, label = percent,
                na.value = grey20k) +
  gisr::si_style_map() 

map_lhs + map_rhs
si_save(here(pub_images, "map_second_iterations"))
...
map <- 
  ...
  labs(title = "VIRGO LEADS TESTING ACHIEVEMENT AT NEARLY 400%",
       caption = caption)
si_save(here(pub_images, "map_fourth_iteration"), plot = map)
```
...
```{r}
  library(patchwork)
   ...
   map_bar <-  bar_graph + labs(caption = NULL) + 
      map_lhs_adorned + 
      labs(title = NULL) + 
      plot_annotation(title = "VIRGO AND SERPENS LEAD TESTING ACHEIVEMENT IN FY49") 
    si_save(here(pub_images, "Map_bar_combined.png"), plot = map_bar )
```

### Heatmaps
A heatmap is a type of visualization that shows magnuitude of a phenomenon as a color in two dimensions. Generally, the x and y dimensions are categorical variables (such as testing modality by partner) and the color is encoded to the quantitative variable being depicted (positivity rate). Hue (shades of color) or color intensity is used to give readers a visual cues about how the phenomenom is clustered across the x and y variables. The Wall Street Journal's [The Impact of Vaccines](https://graphics.wsj.com/infectious-diseases-and-vaccines/) is an excellent use case of the heatmap. 
...
```{r}
  ...
  si_save(here(pub_images, "heatmap_iteration1.png"))
```
This first iteration is ok but there are quite a few things we can do to improve the readability of the graphic. We'll start by i) moving the primepartner names to the top using the `scale_x_discrete()`, ii) then we will add a white stroke around each box by passing a color argument to the `geom_tile()` line, iii) finally, we can add a `geom_text()` to each tile to help the reader more easily understand how the color maps to the positivity rate. 
...
```{r}
    ...
    si_save(here(pub_images, "heatmap_iteration2.png"))
```
This looks much better. But, we could make the text a bit more readible by using a darker text color for squares with low positivity rates. We can also add a sublte grayish background to help bound the squares a bit. We can also experiment with the `coord_fixed()` option from ggplot. This will keep our tiles a fixd shape. This option can be useful when you want to squish rectangles back into squares. Finally, to ensure that our x-axis title do not overlap we can use the `guide_axis()` function in the `scale_x_discrete()` call to make sure x-axis titles are dodged (non-overlapping).
```{r}
  ...
  df_heatmap %>% 
    ...
    si_save(here(pub_images, "heatmap_iteration3.png"), scale = 1.5)
